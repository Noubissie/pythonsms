...
#                     if results[result].sequence == '1' and results[student].sequence == '2' and results[result].subject == results[student].subject and \
#                             results[result].student_name == results[student].student_name:
#                         if subject_and_mark.get_class_subject_sequence_student(clas=clas, sequence=sequence,
#                                                                                subject=results[result].subject,
#                                                                                name=results[student].student_name) is None:
#
#                             mark = (float(results[result].mark) + float(results[student].mark)) / 2
#                             save = subject_and_mark(student_class=clas, student_name=results[student].student_name,
#                                                     sequence=sequence, \
#                                                     staff_name=results[student].staff_name, subject=results[student].subject, mark=mark,
#                                                     coefficient=results[student].coefficient,
#                                                     competence=results[student].competence + results[result].competence)
#                             CREUD.save_to_database(save)
#                             break
#                         else:
#
...
#                                                                                 subject=results[result].subject,
#                                                                                 name=results[student].student_name).mark = mark
#                             db.session.commit()
#                             break
#                     if results[result].sequence == '1':
#                         if subject_and_mark.get_class_subject_sequence_student(clas=clas, sequence=sequence,
#                                                                                subject=results[result].subject,
#                                                                                name=results[
#                                                                                    result].student_name) is None:
#
#                             save = subject_and_mark(student_class=clas, student_name=results[result].student_name,
#                                                     sequence=sequence, \
#                                                     staff_name=results[result].staff_name,
#                                                     subject=results[result].subject,
#                                                     mark=results[result].mark,
#                                                     coefficient=results[result].coefficient,
#                                                     competence=results[result].competence + results[result].competence)
#                             CREUD.save_to_database(save)
#                         else:
#                             subject_and_mark.get_class_subject_sequence_student(clas=clas, sequence=sequence,
#                                                                                 subject=results[result].subject,
#                                                                                 name=results[
#                                                                                     result].student_name).mark = \
#                                 results[result].mark
#                             db.session.commit()
#                     if results[result].sequence == '2':
#                         if subject_and_mark.get_class_subject_sequence_student(clas=clas, sequence=sequence,
#                                                                                subject=results[result].subject,
#                                                                                name=results[
#                                                                                    result].student_name) is None:
#
#                             save = subject_and_mark(student_class=clas, student_name=results[result].student_name,
#                                                     sequence=sequence, staff_name=results[result].staff_name,
#                                                     subject=results[result].subject, mark=results[result].mark,
#                                                     coefficient=results[result].coefficient,
#                                                     competence=results[result].competence + results[result].competence)
#                             CREUD.save_to_database(save)
#                         else:
...
#                                                                                 subject=results[result].subject,
#                                                                                 name=results[
#                                                                                     result].student_name).mark = results[result].mark
#                             db.session.commit()
#
#                 # if sequence == 'Second Term|Deuxieme tremestre':
#                 #     if results[result].sequence == '3' and results[student].sequence == '4' and results[result].subject == results[student].subject and \
#                 #             results[result].student_name == results[student].student_name:
#                 #         if subject_and_mark.get_class_subject_sequence_student(clas=clas, sequence=sequence,
#                 #                                                                subject=results[result].subject,
#                 #                                                                name=results[student].student_name) is None:
...
#                 #             mark = (float(results[result].mark) + float(results[student].mark)) / 2
#                 #             save = subject_and_mark(student_class=clas, student_name=results[student].student_name,
#                 #                                     sequence=sequence, staff_name=results[student].staff_name,
#                 #                                     subject=results[student].subject, mark=mark,
#                 #                                     coefficient=results[student].coefficient,
#                 #                                     competence=results[student].competence + results[result].competence)
#                 #             CREUD.save_to_database(save)
#                 #             break
...
#                 #     if results[result].sequence == '5' and results[student].sequence == '6' and results[result].subject == results[student].subject and \
#                 #             results[result].student_name == results[student].student_name:
#                 #         if subject_and_mark.get_class_subject_sequence_student(clas=clas, sequence=sequence,
#                 #                                                                subject=results[result].subject,
#                 #                                                                name=results[student].student_name) is None:
...
#
#
# def report(clas="TROISIEME ALLEMAND", sequence="First term|Premiere tremestre"):
...
#
#     # f = open("test.html", "w+")
#     with open("report_card/TROISIEME ALLEMAND.html", "w+") as myfile:
#         myfile.write(output)
#     pdfkit.from_file("report_card/TROISIEME ALLEMAND.html", "report_card/TROISIEME ALLEMAND.pdf")
#
# report()
#
...
def report(clas="TERMINALE ESPAGNOL", sequence="First term|Premiere tremestre"):
    student_report_card = subject_and_mark.get_class_only(clas)
    student_class_report = subject_and_mark.get_class_and_subject_and_sequence_list
    class_info = school_information.change_subjects_of_a_class(clas)
    student_i = student_information.get_students_in_a_class(classs=clas)
    ...
    # f = open("test.html", "w+")
    with open("report_card/TERMINALE ESPAGNOL.html", "w+") as myfile:
        myfile.write(output)
    pdfkit.from_file("report_card/TERMINALE ESPAGNOL.html", "report_card/TERMINALE ESPAGNOL.pdf")
